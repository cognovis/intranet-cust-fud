...
set menu_label "etm-bh_al-compdwld"
...
SELECT DISTINCT ON ($cust_prov_incost) 
   ...
   comp.bank,
   comp.payment_email
   FROM im_costs c, im_companies comp, im_invoices i
   WHERE  $cust_prov_incost = comp.company_id 
	  AND c.cost_type_id = $cost_type 
	  AND c.cost_id = i.invoice_id
          $agent_where
   GROUP BY $cust_prov_incost, c.effective_date,c.payment_days,c.cost_name,i.payment_method_id,comp.main_office_id,comp.company_type_id, comp.main_office_id,comp.bank_sort_code, comp.bank_accno,comp.bank_swift,comp.bank_iban,comp.vat_number, comp.bank,comp.payment_email
   ORDER BY $cust_prov_incost, c.effective_date
"

set sql "
SELECT DISTINCT ON  (o.company_id) 
  --$auto_kto_nr as kontonummer, 
  $bh_acc as kontonummer,
  --case 	when comps.company_type_id in (11000010, 11000011, 57,10246) then '10000000' + comps.company_id 
--	when comps.company_type_id in (11000327,56,58) then '60000000' + comps.company_id 
--    	else NULL
--    end as kontonummer_neu,
  ...
  case when comps.company_type_id = '11000010' then cont.last_name 
			else NULL 
			end as name,
  case when comps.company_type_id = '11000010' then cont.first_names 
			else NULL 
			end as vorname,
  NULL as zusatz,
...
FROM 
  ...
  LEFT OUTER JOIN (
	select	distinct on (object_id_one) 
		r.object_id_one,im_name_from_id(r.object_id_one),r.object_id_two,
		p.first_names,p.last_name
	from	acs_rels r,persons p
	where 	rel_type = 'im_company_employee_rel' 
		and p.person_id = r.object_id_two
	group by r.object_id_one,r.object_id_two,p.person_id,p.first_names,p.last_name
		) cont ON (comps.company_id = cont.object_id_one)	
WHERE 
...
set header0 {"Personenkontonummer" "Erloeskonto" "Aufwandskonto" "Adresstyp" "Anrede" "Nachname" "Vorname" "Unternehmensname" "Laenderkennung nach ISO-Code fuer USt-ID" "USt-ID ohne Laenderkennung" "Steuernummer" "Adresse" "Postleitzahl" "Ort" "Land" "Laenderkennung nach ISO-Code für Bankverbindung" "Bankname" "Kontonummer" "BLZ" "BIC" "IBAN" "IBAN korrekt" "Zahlungs-E-Mail" "Zahlungstraeger"}
set footer0 {}
