...
set help_text "
	<strong>KW Report:</strong><br>
	Bitte im Filter bei <i>Week</i> einstelligen Nummern eine <b>0</b> voranstellen, also <b>03</b> und nicht <b>3</b>
"
...
set report_sql "
	...
	where
		...
		and p.project_nr LIKE ('908%')
		and parent_id is null
		and p.end_date >= :start_date
		and p.end_date <= :end_date
		and (SELECT EXTRACT (WEEK FROM p.start_date)) = :week
		and (SELECT EXTRACT (YEAR FROM p.start_date)) = :year
		-- $customer_sql
	order by
		
		(SELECT EXTRACT (WEEK FROM p.start_date)),
		lower(p.project_nr)
"
